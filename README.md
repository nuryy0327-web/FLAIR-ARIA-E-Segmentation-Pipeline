...
- 005_Mod_H : (0.49, NaN)
<br>
에서 메타데이터에 포함되지 않은 005_Mod_H의 slicespacing값이 0.49보다 높을것이기 때문입니다. 따라서  

- SliceSpacing 데이터가 존재하는 경우 -> 기존 pixelspacing과 10% 이내로 차이나면 그대로 사용
- SliceSpacing 데이터가 존재하지 않는 경우 -> study들을 관찰한 결과 보통 512x312의 비율을 가짐 -> resize(interpolation)
  
 <br>
하는 알고리즘을 통해 모든 데이터를 axial slice로 관찰하였습니다.

---

다만 모델링 파트에선 메모리와 데이터 양의 한계를 고려해, 2.5D 모델로 슬라이스 별 예측을 수행한 뒤 예측 결과를 study 단위의 3D volume [slice, row, column]으로 합산하는 방식을 택했습니다. 
