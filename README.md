...
즉, 모델 1은 inference 시 입력되는 데이터를 자동 전처리해주는 효과가 있습니다. (edema 논의와 무관한 사진은 masking 작업 없이 종료)
...
- **Reference mask 생성**: 의사 annotation 대신 "border 제거 후 상위 1% 밝기 픽셀 = edema 후보"로 가정했습니다.
...
- **데이터의 한계**: 데이터의 양이 부족하며, 정확한 mask가 없었습니다.

---

## 실제 업무라면 추가로 확인하거나 질문하고 싶은 사항

- **Annotation 기준**: ARIA-E로 판독하는 정확한 임상 기준, WMH와 ARIA-E 구분 방법이 궁금합니다. 손수 masking된 데이터를 많이 확보하기 어려울 것 같은데, 로직에 의해 구현되는지 궁금합니다.
